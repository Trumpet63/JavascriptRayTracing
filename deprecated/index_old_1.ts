...
let x3 = {x: 0, y: 0, z: 0};
let x4 = {x: 1, y: 0, z: 0};
let camera = {x: 0, y: 0, z: -1}; // camera

let circles = [
    {center: {x: 0.5, y: 0.5, z: 2}, radius: 0.5},
];
...
function draw(currentTimeMillis: number) {
    ...
    // Color the pixel based on the x,y of the direction vector
    // for (let k = 0; k < rays.length; k++) {
    //     let r = Math.round(mapLinear(xMin, rays[k].ray.x, xMax, 0, 255));
    //     let g = Math.round(mapLinear(yMin, rays[k].ray.y, yMax, 0, 255));
    //     let b = 255 - r;

    //     let pixelOffset = 4 * (rays[k].pixel.j * width + rays[k].pixel.i);
